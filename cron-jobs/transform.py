...
Reads JSON files from $POLLING_TMP (default tmp/polling/, saved by the agent
from MCP calls), transforms them into the format expected by processar.py,
and writes to $LEADS_INPUT_PATH (default tmp/polling/leads_input.json).
...
# TMP isolável por job via POLLING_TMP (evita corrida quando duas sessões de
# jobs diferentes escrevem tmp/polling/{cora,meta_*}.json ao mesmo tempo —
# arquivos truncados e saves desaparecendo entre escritas). Fix 2026-09-14.
...
def main():
    ...
    for source_name, headers in HEADERS.items():
        ...
        # {"values": ...}). Aceitar ambos os formatos.
