#!/usr/bin/env python3
# Relatórios Semanais — CENTROVITA BRAÇO DO NORTE (ILPI) — v1.0, 2026-09-22 (motor v1.4 Bem Viver)
# Uso: python3 relatorio_centrovita_bn.py [YYYY-MM-DD]  (default: hoje)
# Gera UM PDF + UM Excel com os 11 relatórios + Comparativo 13 meses em PDF PRÓPRIO PAISAGEM.
# Fonte: API Controlle v1 (token Centrovita BN). Envio: segunda-feira 14:30 → raulroliveira@hotmail.com
#
# Diferenças vs Bem Viver/Araranguá: NÃO usa centro de custo (conta Controlle dedicada);
# TAG Boleto Emitido id 125164; NÃO tem categorias 01.97/01.99 (somatório "excluindo" some quando vazio).
...
TOKEN = os.environ.get("CONTROLLE_TOKEN_CENTROVITA_BN") or (open(_token).read().strip() if os.path.exists(_token) else "")
...
TAG_BOLETO = 125164  # "Boleto Emitido" (Centrovita Braço do Norte)
...
# Centrovita Braço do Norte NÃO usa centro de custo (conta Controlle dedicada) —
# o filtro padrão é só excluir transferências entre contas.

def ok_bv(t):
    """Filtro padrão Centrovita BN: sem transferências entre contas (sem filtro de centro de custo)."""
...
# ===== dados =====
# Filtro padrão em TODOS os relatórios: sem transferências entre contas (sem filtro de CC).
...
P = Paragraph
ARQ_PDF = f"artifacts/{HOJE.strftime('%y%m%d')}_Relatorios_Centrovita_BN.pdf"
doc = SimpleDocTemplate(ARQ_PDF, pagesize=A4, leftMargin=1.5*cm, rightMargin=1.5*cm, topMargin=1.3*cm, bottomMargin=1.3*cm)
E = []

E.append(P("<b>CENTROVITA BRAÇO DO NORTE — Relatórios Gerenciais Semanais</b>", h1))
...
ARQ_COMP = f"artifacts/{HOJE.strftime('%y%m%d')}_Comparativo_Centrovita_BN.pdf"
...
C = []
C.append(P("<b>CENTROVITA BRAÇO DO NORTE — Comparativo dos últimos 13 meses</b>", h1))
...
if excluir:  # só existe na BN se as categorias de perda forem usadas um dia
...
ARQ_XLSX = f"artifacts/{HOJE.strftime('%y%m%d')}_Relatorios_Centrovita_BN.xlsx"
wb = Workbook()
wb.remove(wb.active)
