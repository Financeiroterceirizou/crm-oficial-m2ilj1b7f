...
TOKEN = os.environ.get("CONTROLLE_TOKEN_CENTROVITA") or (open(_token).read().strip() if os.path.exists(_token) else "")
...
TAG_BOLETO = 131714  # "Boleto Emitido" (Centrovita)
...
CC_CENTROVITA = 170575  # centro de custo Centrovita Araranguá — filtro obrigatório em todos os relatórios

def ok_bv(t):
    """Filtro padrão Centrovita: rateio no centro de custo Centrovita Araranguá e sem transferências entre contas."""
    if not any(c.get("cost_center_id") == CC_CENTROVITA for c in (t.get("apportionments_cost_center") or [])):
        return False
...
P = Paragraph
ARQ_PDF = f"artifacts/{HOJE.strftime('%y%m%d')}_Relatorios_Centrovita.pdf"
doc = SimpleDocTemplate(ARQ_PDF, pagesize=A4, leftMargin=1.5*cm, rightMargin=1.5*cm, topMargin=1.3*cm, bottomMargin=1.3*cm)
E = []

E.append(P("<b>CENTROVITA ARARANGUÁ — Relatórios Gerenciais Semanais</b>", h1))
...
ARQ_COMP = f"artifacts/{HOJE.strftime('%y%m%d')}_Comparativo_Centrovita.pdf"
...
ARQ_XLSX = f"artifacts/{HOJE.strftime('%y%m%d')}_Relatorios_Centrovita.xlsx"
wb = Workbook()
wb.remove(wb.active)
