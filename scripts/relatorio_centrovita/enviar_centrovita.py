#!/usr/bin/env python3
# Envio dos Relatórios Semanais CENTROVITA ARARANGUÁ por e-mail (Resend).
# Uso: python3 enviar_centrovita.py [caminho-do-pdf]
#   Sem argumento: usa o PDF de hoje (artifacts/YYMMDD_Relatorios_Centrovita.pdf).
# Chave Resend: scripts/95b7f382c0a1ba1d/resend_key.txt (fora do repo) ou env RESEND_API_KEY.
# Destinatários: raulroliveira@hotmail.com + financeirodoresidcentrovita@gmail.com (to)
#   + financeiro@terceirizou.com.br (bcc).
# Anexos: Comparativo (paisagem) + Relatórios (retrato) + Excel.
# Idempotência: relatorio-centrovita-AAAAMMDD-mtime-anexos-destinos — regeneração do PDF OU mudança
...
TO = ["raulroliveira@hotmail.com", "financeirodoresidcentrovita@gmail.com"]
...
if len(sys.argv) > 1:
    ...
    pdf_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "artifacts", f"{date.today().strftime('%y%m%d')}_Relatorios_Centrovita.pdf")
...
comp_path = pdf_path.replace("_Relatorios_Centrovita.pdf", "_Comparativo_Centrovita.pdf")
...
html = """<p>Boa tarde!</p>
<p>Segue em anexo o <b>pacote de relatórios gerenciais semanais da CENTROVITA ARARANGUÁ</b> (fonte Controlle).</p>
...
payload = {
    ...
    "to": TO,
    "bcc": BCC,
    "subject": f"Relatórios Gerenciais Semanais — CENTROVITA ARARANGUÁ — {date.today().strftime('%d/%m/%Y')}",
    "html": html,
    "attachments": anexos,
}
...
req.add_header("Idempotency-Key", f"relatorio-centrovita-{hora_pdf}-{len(anexos)}-{destinos}")
